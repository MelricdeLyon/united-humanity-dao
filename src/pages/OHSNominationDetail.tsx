...
import OHSNominationTimeline from '@/components/ohs/OHSNominationTimeline';
...
const OHSNominationDetail = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [nomination, setNomination] = useState<OHSNomination | null>(null);
  ...
  const fetchNomination = async (nominationId: string) => {
    try {
      ...
      // Auto-switch to current round tab if nomination is active
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <Card className="mb-8">
          <CardHeader>
            <div className="flex justify-between items-start">
              <div>
                <CardTitle className="text-2xl mb-2">{nomination.title}</CardTitle>
                <CardDescription className="text-lg">{nomination.description}</CardDescription>
                <div className="flex items-center gap-4 mt-4">
                  <div className="flex items-center gap-2">
                    <Trophy className="h-4 w-4" />
                    <span>{getPositionTitle(nomination.position, nomination.region)}</span>
                  </div>
                  {nomination.region && (
                    <div className="flex items-center gap-2">
                      <Users className="h-4 w-4" />
                      <span>Région: {nomination.region}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <CardContent>
            <OHSNominationTimeline election={nomination} />
          </CardContent>
        </Card>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-5">
            ...
            <TabsTrigger value="round1" disabled={nomination.status === 'upcoming'}>
              Tour 1 - Nominations
              {nomination.current_round === 1 && nomination.status === 'active' && (
                <CheckCircle className="ml-1 h-3 w-3 text-green-500" />
              )}
            </TabsTrigger>
            <TabsTrigger value="round2" disabled={nomination.current_round < 2}>
              Tour 2 - Présélection
              {nomination.current_round === 2 && nomination.status === 'active' && (
                <CheckCircle className="ml-1 h-3 w-3 text-green-500" />
              )}
            </TabsTrigger>
            <TabsTrigger value="round3" disabled={nomination.current_round < 3}>
              Tour 3 - Vote Final
              {nomination.current_round === 3 && nomination.status === 'active' && (
                <CheckCircle className="ml-1 h-3 w-3 text-green-500" />
              )}
            </TabsTrigger>
            <TabsTrigger value="results" disabled={nomination.status !== 'completed'}>
              Résultats
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="round1">
            <OHSNominationRound election={nomination} />
          </TabsContent>

          <TabsContent value="round2">
            <OHSPreselectRound election={nomination} />
          </TabsContent>

          <TabsContent value="round3">
            <OHSFinalRound election={nomination} />
          </TabsContent>
...
export default OHSNominationDetail;